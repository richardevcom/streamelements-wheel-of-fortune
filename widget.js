...
function startSpin() {
	if (wheelSpinning === false) {
		...
		let beforeSpinEl = document.querySelector('#container.beforeSpin');
        if (beforeSpinEl.classList.contains('beforeSpin')) {
          
          beforeSpinEl.classList.remove('beforeSpin');
        }
        ...
        setTimeout(function () {
			let afterSpinEl = document.querySelector("#container")
			if (!afterSpinEl.classList.contains("beforeSpin")) {
				afterSpinEl.classList.add("beforeSpin")
			}
		}, 7 * 1000 + 100)
	}
}
